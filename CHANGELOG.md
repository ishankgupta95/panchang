...
Ekadashi-split and regional-new-year fixes below, fixed against
reference-almanac evidence (58 day-pages Jaipur-Feb-2027 / Kolkata-Nov-2026, 14
choghadiya pages, the full 2024-2028 Ekadashi corpus, plus targeted year-page
sweeps). Carries two deliberate breaking type changes
(`inauspicious.durMuhurta`, `muhurtas.amritKala`), following the varjyam
`TimePeriod[]` precedent in part 1 below.
...
- **`inauspicious.durMuhurta` is now `DurMuhurtaPeriod[]`** (was
  ...
  'night'`. The old table was wrong on **all seven weekdays**; the almanac (58
  ...
  and returns the tagged windows. Tier-1 pin: one full almanac week (Jaipur,
  Feb 1-7 2027, ±2 min).
- **`muhurtas.amritKala` is now `TimePeriod[]`** (was `TimePeriod | null`). The
  old model (sunrise-anchored, ahoratra-elastic ghatikas, one window, dropped
  when crossing next sunrise) disagreed with the almanac by up to ~16 h. The
  almanac's Amrit Kalam is the **varjyam architecture**: anchored at the
  ...
  window **starts** in, 0-2 windows/day. The 27-offset table was recovered from
  54 almanac windows covering all 27 nakshatras (spread ≤0.1 ghati). Corpus
  note: ProKerala's Telugu panchangam independently confirms the architecture
  and most offsets (7 of 9 sampled windows ≤2 min) but implies Mula ≈ 45 and
  U.Bhadrapada ≈ 47.5 where the almanac uses 44 / 48, and attributes
  post-midnight windows to the calendar day; the almanac is the project's parity
  bar, so its table and attribution ship. `computeAmritKala` is replaced by
  `computeAmritKalaWindows(sunriseUtc, nextSunriseUtc, getMoon)`. Tier-1 pin: 16
  days across both sweep cities, incl. the almanac's empty day (Jaipur
...
- **Night choghadiya (CH-1):** the 2026-08-13 capture transposed Kaal/Labh
  ...
  Verified on 14 consecutive almanac nights across Bengaluru-Feb-2027 and
...
- **Bhadra vasa (BH-1):** vasa now follows the classical Moon-rashi rule
  ...
  Swarga; Kanya/Tula/Dhanu/Makara → Patala) instead of the tithi-half table, and
  `BhadraInfo` gained a piecewise `vasa` segment list for mid-window Moon
  transitions (the almanac at Ujjain 2026-08-19: "Patala upto 02:30 AM, then
  Swarga", reproduced to the minute). Top-level `location` = vasa at window
  start; window detection and times untouched.

### Fixed: calendar and festival dates

- **Sankranti day rule (SK-1):** a transit during daylight keeps its civil day;
  a transit between sunset and the next sunrise is observed on the NEXT
  sunrise's day (was: the Hindu day containing the transit). The almanac's full
  2027 table discriminates it (Makara Jan 14 21:14 IST → observed Jan 15; Tula
  Oct 18 02:12 → Oct 18; Vrishchika Nov 17 02:02 → Nov 17) and is pinned (day +
  moment ≤2 min) as a tier-1 test. Night-transit rows in 2025/2026 move
  accordingly (7 of 24); all daytime rows are unchanged. `SankrantiEvent` now
  exposes the transit instant as `moment`. Applied to `computeSankrantisForYear`
  and every festival emission keyed on the transit (`sankranti`,
  `makar_sankranti`, `pongal`, regionals, Lohri / Raja Parba neighbors).
- **Kshaya-Dwadashi Ekadashi advance (EK-1):** when the Dwadashi after an
  Ekadashi contains no sunrise there is no valid parana morning, and the Smarta
  fast advances to the day the tithi begins (Trisprisha), with the Vaishnava
  (Gauna) fast on the udaya day. The almanac in 2027: Pausha Putrada Jan 18 +
  "Trisparsha Mahadwadashi / Gauna" Jan 19; in 2025 (Devutthana Nov 1 / Nov 2)
  it shows the same split. The 2026 list is untouched (tier-1 re-run green); the
  almanac's full 25-date 2027 list is now pinned.
- **Vijayadashami (FE-2):** now keyed on aparahna, not sunrise: the day
  ...
  from 11 almanac years (2020-2030) and pinned: includes 2026 Oct 20 and
  2027 Oct 9, both of which the sunrise rule mis-dated by one day.
- **Karva Chauth (FE-3):** now keyed on the Chaturthi prevailing at MOONRISE
  (unclamped Hindu-day moonrise), with a sunrise-prevalence fallback when the
  tithi touches no moonrise on either day (in 2025 the Chaturthi missed both
  moonrises and the almanac still printed Oct 10). The almanac's 2024-2027 dates
  are pinned; 2027 moves Oct 19 → Oct 18. Sankashti Chaturthi gained the same
  vriddha two-moonrise dedupe.
- **Krishna Janmashtami (FE-1):** the Smarta ladder recovered from 7 almanac
  years (2024-2030): the udaya-Ashtami day wins whenever Ashtami OR Rohini
  touches its nishita muhurta; otherwise the day Ashtami covers nishita wins,
  Saptami-viddha or not (2025). Rohini alone never pulls the festival onto a
  Navami day (2028). 2027 moves Aug 24 → Aug 25 (the almanac's main date); 2026
  stays Sep 4.
- **Kali Yuga year (CV-1):** now increments at Chaitra Shukla Pratipada
  ...
  mislabeled every date in [Feb 18, Chaitra Pratipada). Almanac pins:
...
Closes the divergence the audit tracked but did not fix. The almanac states the
orientation in prose on every Ekadashi date-time page: *"Smartha with family
should observe fasting on first day only. The alternate Ekadashi fasting, which
is the second one, is suggested for Sanyasis, widows and for those who want
Moksha. When alternate Ekadashi fasting is suggested for Smartha it coincides
with Vaishnava Ekadashi fasting day."* So in every split the almanac publishes,
whatever its tithi geometry, **the earlier day is Smarta and the later day
Vaishnava**.

A sweep of all 109 Ekadashi tithi windows in 2024-2028 (Jaipur) against
the almanac's year lists found exactly three geometries that make it print
two days. The library now agrees with all three: 151 emission-days compared,
zero mismatches.

- **Dashami-viddha split was emitted backwards (VE-1):** the library put the
  Vaishnava fast on the viddha day and deferred the *Smarta* fast to Dwadashi.
  It is the Vaishnava observance that rejects an arunodaya-viddha Ekadashi; the
  Smarta fast keeps the udaya-vyapini day. Now: viddha day → `smarta_ekadashi` +
  `ekadashi` (no `vaishnava_ekadashi`); next day → `vaishnava_ekadashi`. Pinned
  on all five pairs the almanac publishes in 2024-2028: Vijaya Mar 6/7 2024,
  Apara Jun 2/3 2024, Papamochani Mar 25/26 2025, Rama Oct 25/26 2027, Aja Aug
  16/17 2028. The 96-min (4-ghati) arunodaya already in use separates them
  cleanly: those five tithis begin 15-79 min before sunrise against a 132-min
  minimum across the 94 non-split days. The kshaya/Gauna and Trisprisha branches
  already had this orientation, so all three split mechanisms now agree.
- **Vriddha-Dwadashi (Pakshavardhini) split was missing entirely (VE-2):** when
  the Dwadashi following an ordinary Ekadashi prevails at two consecutive
  sunrises, the almanac moves the alternate (Vaishnava) fast onto that Dwadashi
  and prints a standalone "Vaishnava &lt;name&gt;" row. The library emitted
  nothing on the second day and both fasts on the first. Now detected from
  sunrise tithis alone on the Dwadashi day (no extra rise/set search) and with
  one lookahead on the Ekadashi day. 4 occurrences in 2024-2028, no
  counterexamples: Nirjala Jun 6/7 2025, Shravana Putrada Aug 23/24 2026, Vijaya
  Feb 20/21 2028, Devutthana Oct 28/29 2028.
...
- `computeEkadashiDatesForYear` is untouched: its sunrise-prevalence list
  already returns the almanac's Smarta dates. The 2026 (24-date) and 2027
  (25-date) tier-1 lists, the kshaya pairs, the vriddha day and the Trisprisha
...
- **`FestivalInfo.deferralDate` is removed.** It was declared on the type and
  ...
  the library ever assigned it, so every read got `undefined`. Its premise is
...
The SK-1 sankranti day rule was never carried into the Mesha-anchored regional
new years: `baisakhi`, `vishu`, `pohela_boishakh`, `puthandu` and `bohag_bihu`
all shared one day, and `getHinduNewYear`'s solar path used its own coarse
"first day already in Mesha at 00:00 UTC" sampling. Three of the four traditions
the almanac publishes key off the transit **moment** differently and land on
different dates in the same year.

Validated against the almanac's per-region date pages for 2025-2029, whose five
Mesha transits (Apr 14 03:30 · Apr 14 09:39 · Apr 14 15:33 · **Apr 13
21:47** · Apr 14 03:56 IST) cover pre-dawn, morning, afternoon and
post-sunset, and without all five the rules are indistinguishable:
...
- The three moved keys emit from their own day flags rather than
  ...
  `tests/validation/regional-solar-newyear-almanac.test.ts` (20 region-years).
...
- **Bohag Bihu (Assam) is not pinned.** The reference almanac publishes no Bohag
  / Rongali Bihu date page, so the Assamese rule could not be established to the
  project's ≥2-source bar and the key still rides the Sankranti observance day
  (as Tamil Nadu). Assamese practice generally follows the Bengali reckoning
  (new year on the day *after* the transit's civil day, with the transit day
  itself as Goru Bihu), which would group it with `pohela_boishakh` and change
  its date in 4 of the 5 years above. Left on the transit day pending a citable
...
- **`inauspicious.varjyam` is now `TimePeriod[]`** (was `TimePeriod | null`).
  The old single-window contract evaluated only the nakshatra active at sunrise
  and dropped the almanac's second Varjyam row on transition days. The field now
  lists every window whose **start** falls in the Hindu day, in start order (the
  almanac's attribution: a window beginning before sunrise belongs to the
...
- **Opt-in Gana-dosha cancellation** in `computeAshtakoot` via a new third
  parameter `options: { ganaCancellation?: boolean }` (default `false`; default
  output is byte-identical, preserving reference-almanac 36-guna parity). When
  raised, a doshic Gana score (≤ 1) is restored to 6 if the two Moons' sign
  lords are the same graha or mutual naisargika friends; the reason is recorded
  in `cancellations`. Conditions sourced from ≥2 independent pandit corpora
  (Truthstar/MysticGazer, AstroSight, JagannathHora); weaker or interpretive
  conditions are documented on `AshtakootOptions` and not encoded.
...
- **Ashtottari Dasha used an invented proportional nakshatra split.** The
  starting lord came from dividing the zodiac in proportion to each lord's years
  from a Krittika anchor (Sun's segment = 1.5 nakshatras), matching no consulted
  source. Replaced with the classical Ardradi group table (malefics rule 4
  nakshatras each, benefics 3; Sun = Ardra…Ashlesha, Venus =
  Krittika…Mrigashira; exported as `ASHTOTTARI_NAKSHATRA_GROUPS`), with the
  balance taken as the elapsed fraction of the lord's group. Verified against
  PyJHora (27-star form) and Maitreya 8 (28-star form with Abhijit): identical
  longitude spans, so both sources agree on every lord-at-birth and balance this
  produces. Mula (alone of the 27 nakshatras) carries two Varjyam spells, at
  elapsed ghatikas 20 and 56 of its duration. Only the 56-spell was tabulated,
  so days where the almanac prints Mula's 20-spell window (e.g. 2026-08-22, and
  2026-09-19 where BOTH land in one day) published nothing. Validated against a
  61-day almanac scrape (Aug-Sep 2026, Ujjain: 62/62 windows match ≤2 min incl.
  display truncation), ProKerala's Telugu panchangam, and B.V. Raman's *Muhurta*
  (Moola tyajya = 20).

**Major release.** Held until the ephemeris port landed so the
...
- **Lahiri ayanamsa corrected by +38″.** The constant sat 38 arcseconds behind
  the reference almanac's; the replacement was solved from its own published
  values across 1950-2050. Nakshatra end-times move ~69 s later, yoga ~129 s
  (it carries the ayanamsa twice), planetary longitudes +0.0106°; tithi and
  karana are unchanged because Moon − Sun cancels the ayanamsa. Almanac drift
  32.3 s → 17.4 s.
...
- **The ephemeris is this library's own, and published values move by about a
  ...
  1900-2100: Sun 1.613″ → **0.323″**, Moon 3.747″ → **1.261″**, and every planet
  between 3× and 23× inside its own ceiling. Measured movement in published
  output, over 241 MB of results spanning 1912 / 2025 / 2088: tithi and karana
  end times ≤776 ms, yoga ≤569 ms, nakshatra ≤368 ms, sunrise- proportional
  windows ≤94 ms, **zero** changes to any index, name, boolean, count or
  festival date. Almanac parity is unchanged or marginally better on all four
  elements (worst 60 s → 58 s).
...
- **Auspicious entries that no hard exclusion could ever let through.**
  ...
  the almanac lists Ekadashi among the six *preferred* vivah tithis, and the
...
- **Sarvartha Siddhi rebuilt from the reference almanac's full 2026 listing.**
  Its page serves one month at a time but takes `?date=DD/MM/YYYY`, so all
  twelve months were pulled: 116 windows. Each window is a nakshatra's span
  clipped to its Hindu day, so the table was *derived* from them rather than
  checked against them. Ten cells changed: **added** Sun + Ashwini, Tue +
  Ashlesha, Wed + Krittika; **removed** Sun + Shravana, Mon + Hasta, Tue +
  Uttara Phalguni, Thu + Swati, Fri + Bharani, Fri + Chitra, Sat + Revati. Each
  of the seven occurred three to five times in 2026, often across most of the
  Hindu day, with no almanac window on any of those dates. The result reconciles
...
- **Griha Pravesh and Vahan Kharidi lists rebuilt against the almanac's
  published calendars.** Three years of its dated shubh-dates pages (2025-2027
  Mumbai; 121 and 311 published muhurat days) falsify several hand-transcribed
  entries. Griha Pravesh: Magha / Hasta / Swati / Shravana appear 0-1 times in
  three years and give way to Mrigashira and Chitra (15-17 each); Saturday (23
  published days, zero almanac weekday rejections) moves from inauspicious to
  auspicious; the `excludeEkadashi` hard veto is dropped (Ekadashi is the
  almanac's third-most-used griha pravesh tithi) and Dashami / Ekadashi become
  auspicious. Vahan Kharidi sheds the three Sthira nakshatras (0 occurrences in
  three years) for the Chara / Mridu set the almanac's own prose names
  (Mrigashira, Chitra, Swati, Dhanishtha, Shatabhisha, 30-37 occurrences each),
  and stops banning Ashtami (44 occurrences) and Purnima (23); Sunday joins the
  auspicious weekdays. Vivah survives the same screen untouched: its eleven
  nakshatras are exactly the almanac's operative set, and it applies no tithi or
  weekday shuddhi to marriage at all. Pinned in
  `tests/validation/almanac-muhurta-lists.test.ts`.
...
- **Special yogas are evaluated across the Hindu day, not at sunrise.** Amrit
  Siddhi, Sarvartha Siddhi, Ravi / Guru Pushya, Jwalamukhi, Dwipushkar,
  ...
  sunrise snapshot found only 5 of the almanac's 8 August Sarvartha Siddhi
  windows. Each (tithi, nakshatra) pair is now evaluated where the two segments
  actually overlap in time, so no combination is scored that never occurs. More
  yogas fire than before, which is the point. `getDailyPanchang` with
  `computeEndTimes: false` keeps the single-snapshot behaviour, since segment
  times are what make overlap checking possible.
- **Amrit Siddhi and Tripushkar validated against the reference almanac's full
  2026 listings** (Mumbai): 24 and 16 occurrences, all reproduced exactly, no
  false positives, pinned in
  `tests/validation/almanac-special-yogas.test.ts`. The almanac dates a window
  by the calendar day its start falls in; this library attributes it to the
  Hindu day, so a window closing at sunrise is listed there on D and reported
  here on D−1. Same interval, different convention.
- **Sarvartha Siddhi confirmed out-of-sample across four city-years.** The
  2026-derived table was re-derived independently from the almanac's 2025 and
  2027 Mumbai listings and its 2026 New Delhi listing: 471 published windows in
  ...
  shubhpanchang variant) never occurs. A handful of the almanac's pre-dawn
  windows turn out to be dated by Hindu day rather than by the start's civil
  date (each start anchors to a nakshatra boundary that exists only on the
  following date), which also corrects the mechanism behind the withdrawn
  ...
  `tests/validation/almanac-special-yogas.test.ts`.
...
tree: `generate/notes/vcompare/driver.mjs`, median of 11 processes per
configuration per implementation, one configuration per process, rotating which
implementation runs first so a scheduling stall lands on all of them. The
`ctrl/*` control rows (pure arithmetic, no ephemeris) come out at 1.00×, which
is the check that says the harness measured the library rather than the machine.
...
in 0.9749 ms, so against it the change is −57.9% rather than −93.2%.*

Two phases of work. **Structural**: Chebyshev interpolation of Sun/Moon
...
- **the rise/set position track fitted over four days instead of one**: the
  ...
  a five-day period), and `generate/notes/track-fit.src.ts` is the sweep that
  says so.
...
- **NASA/Espenak eclipse ground truth committed**: the Five Millennium Canon
  ...
  circumstances at 10 sites). The second closes what had been the thinnest
  Tier 0 coverage: a geocentric canon cannot say when the partial phase begins
  at Varanasi, and a differential test against the package being removed would
  have carried no authority.
...
**Minor release: eclipses + moon-phases static tables, plus an
...
- **`panchang-ts/eclipses`**: bundled, engine-free eclipse table for India
  ...
  eclipses carry no `sutak` and are not religiously observed (almanac / pandit
  consensus).
...
- **Adhika (leap) masa detection**: `getDailyPanchang`'s `chandraMasa.isAdhika`
  ...
  boundaries, e.g. Adhik Jyeshtha (17 May to 15 Jun 2026). Internal change only;
...
**Minor release: Pathu Porutham + Narayan Dasha
+ KP sub-lord layer + Prashna foundation.** South-Indian regional
features that the v3.3 surface skipped.
...
- **`computePathuPorutham`**: Tamil/Kerala 10-fold marriage
  ...
  is binary-scored (pass / fail) per the AstroVed / Tamil-almanac
  convention; aggregate is a 0..10 count. Three vetoes (Yoni, Rajju,
  Vedha) flip `recommended` to false regardless of count. Reuses the
  existing NAKSHATRA_YONI / NAKSHATRA_GANA / RASHI_VASHYA / NAISARGIKA_MAITRI
  ...
  Ashtakoot, so call both side-by-side.
...
- **`computeKpSubLord` / `computeKpCuspalSubLords` /
  ...
  `houseSystem: 'placidus-kp'` regardless of caller-supplied option, since
...
locale-resolved (consistent with the existing NAKSHATRA_YONI tables, the
yoga catalog and the Saham names).

## 3.3.0

**Minor release: Varshaphala + Tithi Pravesha + Arudha
...
**Minor release: Ashtakavarga + Yogas + Karakas + Bhava Bala.**
...
None. This release is fully additive: every v3.1 export keeps the same shape.
...
**Minor release: Advanced Astrology + Muhurta Engine.**
...
- Muhurta stock-rule numerics (auspicious / inauspicious tithi /
  ...
  Charak's *Predictive Astrology* Ch. 23, and cross-checked against the
  reference almanac's muhurat pages. Regional traditions vary; pass a custom
  rule for strict-region parity.

### Breaking changes

None. The surface added here is entirely new: no v3.0 export was renamed,
...
**Patch: README rewrite for v3.x.** Documents the v3.0 birth-chart
...
**Major release: Birth Chart Foundation.** Adds the
...
- **True Rahu / Ketu node**: `computePlanetaryPositions(...,
  ...
  `'mean'`, so no behaviour change for existing callers.
...
- **Ashtakoot**: 30+ pairs cross-validated against the reference almanac's
  horoscope-match calculator (per-koot tolerance ±1).
...
**Minor release: Panchaka Rahita + Do Ghati Muhurta.** Closes the
dainika-parity work; backwards compatible (additive to `DailyPanchangResult`
only).
...
The reference almanac's Do Ghati table presents the **same 30-name sequence on
every weekday**: there is **no vara-based rotation** of the kind Choghadiya /
Gowri Panchangam use. Verified against its published do-ghati-muhurat table for
Wed 2026-04-15 and Mon 2026-04-20 producing identical name sequences. This
matches the classical Brahmana / Smriti enumeration where each muhurta is
associated with a fixed presiding deity independent of the day of the week.
`computeDoGhati` therefore takes no `varaIndex` parameter; sourcing is cited
inline in [src/core/doGhati.ts:3-21](source/ts/src/core/doGhati.ts#L3-L21).
...
- `doGhatiNames: readonly string[30]` added to `Translations` and to
  ...
  indices 15-29 (matches the reference almanac's published order).
...
v2.4 work; was 5,149 in v2.1.0 → **+899**). New files added by
...
**Minor release: Anandadi Yoga + six classical
...
- **Six new entries in `SpecialYogaInfo[]`**: the existing four
  ...
  - `ravi`: auspicious; Moon-from-Sun nakshatra-distance in the
    ...
    (per the reference almanac's published occurrence list).
...
**Minor release: Tarabala, Varjyam, Ganda Mula,
Madhyahna + Pratah/Sayahna Sandhya, Dinamana/Ratrimana labels.**
Backwards compatible. Closes the bulk of the visible gap with the reference
almanac's dainika panchang panel.
...
- **Varjyam**: forbidden ~96-min window per day, nakshatra-keyed. Uses
  ...
  [src/utils/constants.ts](source/ts/src/utils/constants.ts)) anchored to nakshatra
...
- **Pratah Sandhya** / **Sayahna Sandhya**: dawn / dusk twilight windows.
  Asymmetric: Pratah ends *at* sunrise, Sayahna starts *at* sunset; both have
  width = `nightDuration / 10` (three nighttime ghatikas). Matches the reference
  almanac's published Sandhya. Two new fields.
- **Dinamana** / **Ratrimana labels**: classical aliases of
  `dayDurationMinutes` / `nightDurationMinutes` exposed as
  `dinamanaMinutes` / `ratrimanaMinutes` for parity with the reference
  almanac's panel labelling.
...
- `VARJYAM_OFFSET_GHATIKAS`: 27-entry per-nakshatra offset table sourced
  ...
  [src/utils/constants.ts](source/ts/src/utils/constants.ts).
...
Legacy region identifiers resolved via [src/core/regionAlias.ts](source/ts/src/core/regionAlias.ts)
...
New unit file [tests/unit/regionAlias.test.ts](source/ts/tests/unit/regionAlias.test.ts)
...
**Major release: correctness fixes + almanac-aligned API.** This release ships
...
3. **Transliteration aligned to the reference almanac.** Public string outputs
...
- **Masik Karthigai** monthly observance added: fires whenever Krittika
  ...
  sunset / nishita sample), matching the almanac's broader rule rather than a
  strict at-sunrise check.

### Test fixtures: provenance overhauled

- `almanac-diaspora.json` was previously library-self-seeded
  (admitted in `_meta.generator: "library-snapshot"`). All 15 entries
  now transcribed from the reference almanac (NYC, London, Sydney, Dubai,
  Singapore × 3 dates each; geoname-ids documented in `_meta.source`).
- `almanac-precise.json` non-sunrise fields (tithi, nakshatra,
  chandramasa) now almanac-verified for all 15 unique dates.
- Renamed the two mis-prefixed structural fixtures to
  `structural-india.json` and `structural-world.json`; these files only
  assert calendar-derived weekday + structural counts, not almanac values.
  The almanac prefix is now reserved for true almanac-sourced fixtures.
- New [testdata/README.md](testdata/README.md) documents the
...
exports, no changed defaults, no changed return shapes. Two new APIs land
(`computeChandraBalam`, `computeVimshottariDashaFromBirth`); everything else
added is docs, tests, and JSDoc only. **One numeric output change** to flag;
...
node). This aligns with classical Vedic practice and the reference almanac's
published values.
...
the reference almanac or other Vedic almanacs, this release will produce
*better* numbers.
...
- 4,864 tests passing
- Almanac-verified across 2025-2026 (Delhi, Chennai, Mumbai, Bangalore, Pune)
- Sunrise/Sunset ≤29 s observed vs the almanac's minute-midpoint (±45 s test
  tolerance)
- Tithi / Nakshatra / Yoga / Karana end-times max 2.01 min drift, ±3 min
  tolerance (20 assertions)
- Planetary positions (Sun-Saturn) ≤0.02° vs the almanac's sidereal values
- 12 almanac-verified festival dates (2025-2026)
- Bundle parses cleanly with the official Hermes JS frontend
  (`npm run test:hermes` via `hermes-parser`)

### Documented tradeoff

Festival detection uses **tithi-at-sunrise**. Festivals that the almanac
resolves via tithi-at-midnight (Krishna Janmashtami, Maha Shivaratri,
Diwali/Lakshmi Puja), madhyahna-vyapini (Ganesh Chaturthi edge years, Akshaya
Tritiya 2026), or kshaya-tithi handling (Ugadi 2026-03-19) can drift ±1 day from
the almanac's canonical date. See the README §Festival Detection for the full
caveat set.
