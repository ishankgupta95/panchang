/**
 * Unit tests for `computeAspects` — planetary aspect (drishti) houses.
 *
 * Algorithmic checks — independent of natal chart computation. We synthesize
 * birth charts with known planetary house placements and verify that the
 * universal 7th aspect plus the malefic special aspects (Mars 4/8, Jupiter
 * 5/9, Saturn 3/10) land on the expected houses.
 *
 * BPHS Ch. 26 ("Drishti Vichar") is the canonical source.
 */
...
/**
 * Build a synthetic D1 chart placing each graha at the requested house.
 * `houseByPlanet` maps planet → house (1..12). Unspecified planets default
 * to house 1.
 */
...
describe('computeAspects — integration with computeRashiChart', () => {
  it('returns valid aspects for a real natal chart', () => {
    ...
    const a = computeAspects(chart);
    // Mars / Jupiter / Saturn always have exactly 3 aspects.
    expect(a.Mars).toHaveLength(3);
    expect(a.Jupiter).toHaveLength(3);
    expect(a.Saturn).toHaveLength(3);
    // Houses are 1..12.
  ...
  it('Rahu and Ketu aspect axially-opposite houses (their 7ths are 6 apart, ie axial)', () => {
    const chart = computeRashiChart(SAMPLE, DELHI);
    const a = computeAspects(chart);
    // Rahu in house R aspects R+6; Ketu in house K = R+6 aspects K+6 = R.
    // Their aspect lists in 7-only mode should each be the other's house.
