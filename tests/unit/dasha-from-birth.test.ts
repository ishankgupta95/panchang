/**
 * Unit tests for the ergonomic Vimshottari Dasha wrapper that accepts a
 * birth date directly (without pre-computed sidereal Moon longitude).
 */
...
describe('computeVimshottariDashaFromBirth', () => {
  ...
  it('returns exactly 9 mahadashas spanning between 100 and 120 years', () => {
    // The first mahadasha is a partial (balance) of the birth-moment dasha,
    // so total span = 120 - startLordYears × elapsedFraction.
    // Worst case: startLord = Venus (20y) with elapsed ~1.0 → total ≈ 100y.
    // Best case:  elapsed = 0 → total = 120y.
