/**
 * Unit tests for `computeKpSubLord`, `computeKpCuspalSubLords`, and
 * `computeKpSignificators` (Step 33-3).
 *
 * Strategy:
 *   1. **Sub-width invariants** — the 9 sub-divisions cover exactly
 *      one nakshatra (13°20'); proportional to Vimshottari years.
 *   2. **Boundary cases** — at the start of a nakshatra the sub-lord
 *      equals the star-lord; at carefully-chosen offsets the sub-lord
 *      transitions to the next planet in the cycle.
 *   3. **Sign / star / sub lord pinning** — synthetic longitudes whose
 *      results are hand-computable.
 *   4. **Cuspal sub-lords** — 12 entries, ascendant cusp = lagna.
 *   5. **Significators** — every planet signifies ≥4 houses (KP rule
 *      of thumb); shape-correctness; round-trip byPlanet ↔ byHouse.
 *   6. **Fixture sweep** — 5 R-tier charts.
 */
...
// ── 3. Cyclic sub-lord wrap when star-lord ≠ Ketu ──────

describe('computeKpSubLord — sub-cycle wraps inside Bharani (star-lord Venus)', () => {
  // Bharani spans 13°20' to 26°40'. Star-lord = Venus (DASHA_ORDER index 1).
  // Sub-cycle starts at Venus, then goes Sun, Moon, Mars, Rahu, Jupiter,
  // Saturn, Mercury, Ketu (wrap). Each width per its dasha-years.
  ...
  it.each(expectedOrder.map((lord, i) => [i, lord] as const))(
    ...
    (i, lord) => {
      // compute middle of the i-th sub-division within Bharani.
      let cum = 0;
      for (let j = 0; j < i; j++) {
...
describe('computeKpCuspalSubLords', () => {
  ...
  it('cusp 1 (ascendant) longitude matches lagna under Placidus-KP (KP ayanamsa)', () => {
    // KP cuspal sub-lords default to 'krishnamurti' ayanamsa (the
    // canonical KP system), so compare against a lagna computed under
    // the same ayanamsa to avoid the Lahiri-vs-KP ~0.08° offset.
    const r = computeKpCuspalSubLords(SAMPLE, DELHI);
  ...
  it('forces Placidus-KP regardless of caller-supplied houseSystem', () => {
    ...
    // Placidus cusps differ from whole-sign, so the sub-lords differ in
    // general — assert at least one cusp's longitude differs from the
    // whole-sign-cusp longitude (lagna's rashi start).
    ...
    // The two should match because both should have used Placidus-KP.
    for (let i = 0; i < 12; i++) {
...
describe('computeKpSignificators', () => {
  ...
  it('most visible grahas signify ≥3 houses (KP rule of thumb)', () => {
    // Strict KP "≥4" rule of thumb breaks when a planet sits in its
    // own nakshatra (rule 2 collapses onto rule 1) or a planet's
    // star-lord owns the same rashis as itself; the union can fall to
    // 1–3 houses in such cases. Assert that at least 5 of 9 planets
    // clear the 3-house bar — the typical KP-text signifying coverage.
    const chart = computeRashiChart(SAMPLE, DELHI);
    const sig = computeKpSignificators(chart);
...
describe('Fixture sweep — KP cuspal sub-lord structural invariants', () => {
  ...
  it.each(SWEEP_NAMES)('%s: most planets signify ≥3 houses (KP rule of thumb)', (name) => {
    ...
    const sig = computeKpSignificators(chart);
    // Strict KP-textbook expectation is ≥4 for visible grahas, but
    // overlaps (e.g. star-lord owning a rashi the planet itself owns)
    // can collapse the union. Asserted sweep: at least 5 of 9 planets
    // clear the 3-house bar.
    const passing = ALL_GRAHAS.filter((g) => sig.byPlanet[g]!.length >= 3).length;
    expect(passing).toBeGreaterThanOrEqual(5);
  });
});
