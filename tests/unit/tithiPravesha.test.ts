/**
 * Unit tests for `computeTithiPravesha` (Step 32-2).
 *
 * Strategy:
 *   1. **Self-consistency assertion** — by construction the praveshTithi
 *      must equal natalTithi. This is the single most important
 *      structural check for Tithi Pravesha; the algorithm is defined
 *      such that this invariant holds within the floating-point
 *      precision of the bisection.
 *   2. **Sun-in-natal-sign assertion** — at the pravesha instant, the
 *      sidereal Sun must lie in the natal Sun's rashi (the "solar return"
 *      anchor of the PVR redefinition).
 *   3. **Sun-Moon difference match** — the Moon-Sun separation at
 *      pravesha must equal the natal separation to within 0.001°.
 *   4. **Window assertion** — the pravesha instant should be within ±15
 *      days of the corresponding solar-return anniversary.
 *   5. **Idempotency** — repeated calls give identical results.
 *   6. **Year stride** — successive years' praveshas are ~365 days apart.
 *   7. **Output shape** — all required fields populated.
 */
...
// ── 1. Tithi preservation invariant ───────────────────
...
describe('computeTithiPravesha — within ~1 month of solar anniversary', () => {
  it.each(FIXTURE_NAMES)(
    ...
    (name) => {
      // The pravesha can be up to one synodic month (~29.5 days) before
      // or after the nominal solar anniversary because of the Sun-in-natal-sign
      // correction (PVR redefinition).
      const { utc, loc } = fixture(name);
      const age = 25;
      const tp = computeTithiPravesha(utc, age, loc);
...
describe('computeTithiPravesha — year-stride consistency', () => {
  it('praveshes for consecutive ages are within one synodic month of 365.25', () => {
    // The tithi-pravesha can shift by up to one synodic month (~29.5d)
    // year-to-year because the Sun-in-natal-sign correction can pick the
    // adjacent tithi-match. Bound the stride to [365.25 - 29.6, 365.25 + 29.6].
    const { utc, loc } = fixture('Sachin Tendulkar');
...
describe('computeTithiPravesha — output structural shape', () => {
  ...
  it('respects ayanamsa option (different ayanamsa → potentially different pravesha)', () => {
    // Different ayanamsa → potentially different natal Sun rashi → possibly
    // different sign-window → up to ±29.5d shift. Just assert both calls
    // succeed and produce structurally valid charts.
    const { utc, loc } = fixture('Narendra Modi');
...
describe('_computeNatalTithiIndexForTest — boundary cases', () => {
  ...
  it('handles wraparound: Moon 5°, Sun 350°', () => {
    // Moon - Sun = -345° → +15° → tithi = floor(15/12) = 1
    expect(_computeNatalTithiIndexForTest(350, 5)).toBe(1);
  });
});

// ── 9. Hand-pinned scenario ───────────────────────────
