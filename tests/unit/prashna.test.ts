/**
 * Unit tests for `computePrashnaChart` (Step 33-4).
 *
 * Strategy:
 *   1. **Output shape parity** — Prashna chart and a natal chart cast
 *      with the same parameters return identical structure (lagna,
 *      bhava, planets).
 *   2. **Default house system is Placidus-KP** — KP horary's anchor.
 *   3. **House-system override honored** — caller can request whole-sign.
 *   4. **Ayanamsa override honored**.
 *   5. **Validation passes through to underlying compute**.
 */
...
describe('computePrashnaChart — parity with computeRashiChart', () => {
  ...
  it('with explicit houseSystem + ayanamsa matches computeRashiChart exactly', () => {
    // Prashna defaults to Krishnamurti ayanamsa; pass an explicit
    // ayanamsa to compare against a parity natal chart.
    const prashna = computePrashnaChart(QUESTION_MOMENT, MUMBAI, {
...
describe('computePrashnaChart — default house system + ayanamsa', () => {
  it('defaults to placidus-kp houses + krishnamurti ayanamsa (KP horary anchors)', () => {
    ...
    // Compare against an explicit lahiri chart — Krishnamurti and
    // Lahiri differ by ~0.08° at J2000-era dates, so the difference
    // is measurable but small. This pins that the default is NOT lahiri.
...
describe('computePrashnaChart — option overrides', () => {
  ...
  it('honors caller-supplied ayanamsa', () => {
    ...
    // Different ayanamsas should produce different sidereal lagnas.
