/**
 * Vara x Tithi yoga tables, and their integration with the muhurta scorer.
 *
 * The tables are transcribed from Ernst Wilhelm's *Muhurta Yogas*, verified
 * cell-for-cell against blog.cosmicinsights.net. Rather than restating the
 * same numbers here (which would only prove the file was copied twice), the
 * structural tests below check the properties the *sources* assert: that
 * Siddha and Amrita rows are whole tithi groups, that the cells Wilhelm marks
 * with an asterisk really do form both polarities, and that every row applies
 * to both pakshas.
 */
...
/** Tithi numbers 1..15 that form `yoga` on weekday `vara`, in Shukla paksha. */
...
describe('computeVaraTithiYogas — table structure', () => {
  /**
   * "Nanda Tithis on Venus' Vara; Bhadra on Mercury's; Jaya on Mars'; Rikta
   * on Saturn's; and Purna on Jupiter's constitute Siddha Yoga."
   */
  it('Siddha assigns whole tithi groups to Tue/Wed/Thu/Fri/Sat', () => {
    ...
    // Siddha does not form on Sun or Mon.
  ...
  /** The malefic tables name single tithis, Sunday → Saturday. */
  it.each([
  ...
  /** Krakacha runs the 6th..12th backwards across Saturday..Sunday. */
...
describe('computeVaraTithiYogas — documented ambiguities', () => {
  /**
   * Wilhelm asterisks exactly these cells as forming both an auspicious and
   * an inauspicious yoga. The function must report both rather than choosing.
   */
  it.each([
    [3, 2, 'Wednesday + 2nd'],
    [3, 3, 'Wednesday + 3rd'],
  ...
  it('returns auspicious matches before inauspicious ones', () => {
    const ys = computeVaraTithiYogas(6, 8); // Saturday + Navami
  ...
  it('returns an empty array when no yoga forms', () => {
    // Sunday + 5th forms nothing in any of the seven tables.
...
describe('muhurta engine — Vara x Tithi integration', () => {
  /**
   * 2026-01-17 is a Saturday on Krishna Chaturdashi (index 28) — a Rikta
   * tithi, so Siddha yoga forms and partly redeems it.
   */
  ...
  it('nets an auspicious yoga against an inauspicious tithi', () => {
    ...
    expect(r.score).toBe(45); // 50 − 15 + 10
  });
  ...
  it('is on by default and can be switched off', () => {
    ...
    expect(off.score).toBe(35); // 50 − 15, no redemption
  });
