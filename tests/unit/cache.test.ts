...
describe('LongitudeCache', () => {
  it('memoizes on the exact instant, not a time bucket', () => {
    ...
    // 35 s apart: the Moon has genuinely moved, and the memo must say so
    // rather than collapsing both onto whichever instant was asked for first.
  ...
  it('is order-independent: the value for an instant never depends on history', () => {
    // The property the whole cache exists to guarantee. Reading a spread of
    // instants in different orders must produce identical values — this is what
    // makes `options.sections` output-neutral and keeps unrelated refactors
    // from shifting published transition times.
