/**
 * Unit tests for Tarabala (transit Moon's 9-tara cycle position from janma nakshatra).
 *
 * Classical rule (Muhurta-chintamani Ch. 4 / BPHS Ch. 71): from the janma
 * nakshatra, the 27 nakshatras partition into nine taras that repeat three times.
 * Three taras — Vipat (3rd), Pratyari (5th), Vadha (7th) — are inauspicious;
...
describe('computeTarabala', () => {
  ...
  describe('cycle repetition (3× across 27 nakshatras)', () => {
    // From janma = Ashwini, transits at offsets 9 and 18 should match offset 0
    // (both are Janma tara). Same for every other tara index.
