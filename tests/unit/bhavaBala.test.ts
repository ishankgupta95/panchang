...
const FIXTURE_PINS: ReadonlyArray<{ name: string; totals: readonly number[] }> = [
  ...
  // Re-pinned 2026-08-13 for the Ojha-Yugma grouping correction (Mercury and
  // Saturn are ODD-sign gainers per BPHS 27.18-19; they had been grouped with
  // the even gainers). Predicted before running: each affected chart moves by
  // exactly ±30 V on the houses lorded by a flipped graha — +30 where the
  // graha's D1 AND D9 are both odd, −30 where both even, 0 mixed. Observed:
  // Modi Saturn(+30: h3,h4) Mercury(−30: h8,h11); Dhirubhai Mercury(−30:
  // h7,h10) Saturn mixed(0); Mukesh Saturn(−30: h3,h4) Mercury mixed(0);
  // Sachin and Tata both fully mixed — unchanged. Every delta matched.
  {
    name: 'Narendra Modi',
    totals: [
      378.445, 367.525, 360.1266, 374.4166, 332.525, 294.155,
      283.0117, 462.0925, 310.6234, 434.1574, 432.8025, 348.0117,
    ],
  },
  {
  ...
  },
  {
    ...
    totals: [
      ...
      288.3524, 163.1985, 537.0814, 337.6424, 495.7367, 346.5083,
    ],
  },
  {
    name: 'Mukesh Ambani',
    totals: [
      70.6604, 819.6296, 24.2438, 64.2438, 718.1996, 82.8004,
