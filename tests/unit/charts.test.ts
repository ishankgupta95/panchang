/**
 * Unit tests for `computeRashiChart` (D1) and `computeNavamsa` (D9).
 *
 * D1 verification: shape consistency, lagna/bhava agreement, planet houses
 * derivable from cusps, retrograde flag preserved.
 *
 * D9 verification: classical navamsa rule applied at every rashi-type boundary.
 *   - Movable signs (Aries, Cancer, Libra, Capricorn): start at own sign.
 *   - Fixed signs (Taurus, Leo, Scorpio, Aquarius): start at 9th from own.
 *   - Dual signs (Gemini, Virgo, Sagittarius, Pisces): start at 5th from own.
 */
...
describe('computeRashiChart (D1)', () => {
  ...
  it('Rahu and Ketu are exactly 180° apart', () => {
    ...
    // House difference should also be 6
...
describe('computeNavamsa (D9) — classical rule per rashi type', () => {
  // Movable: Aries, Cancer, Libra, Capricorn → starts at own.
  it('Aries 0° → Aries 0°', () => {
  ...
  // Fixed: Taurus, Leo, Scorpio, Aquarius → starts at 9th from own (= +8).
  ...
  // Dual: Gemini, Virgo, Sagittarius, Pisces → starts at 5th from own (= +4).
  it('Gemini 0° → Libra (6)', () => {
  ...
  it('Boundary at Aries 3°20\' is inclusive of the next nav (Taurus)', () => {
    // The classical division says 0-3°20' = Aries, 3°20'-6°40' = Taurus, etc.
    // At exactly 3.333° we should land in Taurus (next nav).
    const eps = 1e-6;
    expect(Math.floor(navLon(30 / 9) / 30)).toBe(1);   // exactly at boundary
    expect(Math.floor(navLon(30 / 9 - eps) / 30)).toBe(0); // just before
  });
});
