/**
 * Published moon.rise is a calendar-day fact.
 *
 * `getMoonrise` searches up to two days ahead, so on the ~13 days a year with
 * no moonrise on the local calendar day it finds the NEXT day's rise. The
 * publishing layer must clamp that to `null` (the documented contract);
 * before the clamp, the same rise instant appeared byte-identical on two
 * consecutive days' results — 13 duplicated pairs per year at every latitude
 * tested. Found 2026-08-13 while cross-checking August 2026 against the
 * Astrological eMagazine's table (their Aug 6/7 column showed the artifact).
 */
...
describe('moon.rise calendar-day contract', () => {
  it('2026-08-06 Ujjain has no moonrise: rise is null, set is that day\'s own', () => {
    ...
    // The fallback pairs moonset from local midnight — the set that actually
    // occurs on Aug 6 (~12:56 IST), not the next lunation's.
  ...
  it('2026: every published rise is on its own calendar day, no duplicates, ~13 null days', () => {
    ...
    // One no-rise day per lunar month: 12 or 13 depending on phase alignment.
