/**
 * The **frozen reference** rise/set solver for Phase 36.3 — the "correct first"
 * half of §36.0 H's *correct first, frozen second, fast third*.
 *
 * It computes exactly what `src/astronomy/riseSet.ts` computes, with every
 * optimization removed:
 *
 * | | shipped | this |
 * |---|---|---|
 * | body position | 11-node Chebyshev interpolant per 4 UTC days | evaluated at **every probe** |
 * | lunar distance | 75-term tier, good to 5 km | the full 469-term series |
 * | nutation | endpoints of the day, linear in between | evaluated at **every probe** |
 * | bracket scan | 12-minute grid with a slope-bounded subdivision | 30-second grid, exhaustive |
 *
 * So the difference between the two is the interpolation error and nothing
 * else, which is the property that makes the differential test in
 * `tests/validation/differential-riseset.test.ts` a measurement rather than a
 * comparison of two guesses.
 *
 * It shares the ephemeris with the shipped path deliberately. The series'
 * *own* accuracy is Tier 0's question and is answered in
 * `tier0-own-sun-moon.test.ts`; asking it again here would confound the two.
 */
...
/**
 * Altitude of the body's upper limb above the refracted horizon, degrees.
 * Zero at rise and at set. Everything is evaluated at `ms` — no caching, no
 * interpolation, no per-day constants.
 */
...
/**
 * Every rise (`+1`) or set (`-1`) inside UTC day `dayIndex`, found by an
 * exhaustive 30-second scan and bisected to a millisecond.
 *
 * 2,880 ephemeris evaluations per day per direction. That is the point: it is
 * slow enough to be obviously right, so the fast one has something to be
 * checked against.
 */
