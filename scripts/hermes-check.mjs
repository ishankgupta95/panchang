// Hermes JS-syntax compatibility check.
//
// Parses the built CJS bundle with `hermes-parser` (the actual Hermes
// frontend, packaged for Node). If the bundle uses syntax Hermes doesn't
// accept, parse fails and the script exits non-zero.
//
// This is a static check — it does NOT compile to bytecode — but it
// catches every JS-syntax incompatibility that bytecode compilation
// would catch (Hermes runs the same parser internally). Sufficient as
// a publish gate.
