/**
 * Lunar eclipses of 1901–2100 against the NASA/Espenak Five Millennium Canon,
 * for whichever implementation PT_MODULE points at.
 *
 * TT is reached using the *fixture's own* deltaTSeconds rather than either
 * library's ΔT model, so this measures eclipse geometry and not a ΔT
 * disagreement — and so both versions are converted identically.
 */
...
const canon = require(new URL('../../tests/fixtures/nasa-eclipses.json', import.meta.url).pathname);
...
for (const row of canon.lunar) {
  ...
  // `magnitude` is the umbral magnitude for umbral eclipses, clamped at 1 for total.
