/**
 * Drik parity for either implementation, on the same fixture and with the same
 * convention the in-repo audit uses (Drik publishes to the minute, so the
 * midpoint of the printed minute is the least-biased reading).
 */
...
const fx = require(new URL('../../tests/fixtures/drikpanchang-verified.json', import.meta.url).pathname);
...
/**
 * v5 publishes a real instant plus an offset-carrying `*Local` string; v4
 * published the instant *already shifted* by the UTC offset, so its
 * `toISOString()` is the local wall clock and must NOT be shifted again. That
 * is the 4.x bug this release fixed, and getting it wrong here reads as a
 * 5h30m parity error at IST rather than as a units mistake.
 */
const localIso = (v) => {
  if (v === null || v === undefined) return null;
  if (typeof v === 'string') return v;   // v5 *Local, already offset-carrying
  return v.toISOString();                // v4, already offset-shifted
};
