/**
 * P10: the canonical per-day lunar cache must return the same EVENT an uncached
 * SearchRiseSet would, on every day and latitude. Compares getMoonrise/getMoonset
 * against a direct SearchRiseSet from the same instant.
 */
...
for (const [name, lat, lon] of LOCS) {
  ...
  // 800 days from 2024, plus 400 days from 1950 and 400 from 2090.
  for (const base of [Date.UTC(2024, 0, 1), Date.UTC(1950, 0, 1), Date.UTC(2090, 0, 1)]) {
    const n = base === Date.UTC(2024, 0, 1) ? 800 : 400;
    for (let i = 0; i < n; i++) {
      // Probe from several starts within the day, which is where jitter came from.
