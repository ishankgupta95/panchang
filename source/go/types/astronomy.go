...
type EclipseInfo struct {
	...
	// the location rather than anywhere on Earth: at its deepest phase with
	// the Sun up, so a total or annular phase wholly below the horizon reads
	// partial. Penumbral, partial or total for a lunar one, by umbral
	// magnitude.
	...
	// peak, 0 to 1, even when the body is below the horizon then (see
	// VisibleFromLocation). For a lunar eclipse it is the umbral cover, so a
	...
	// horizon or not, and not capped: above 1 when the peak is total. For a
	// lunar eclipse it is the umbral magnitude, which is zero or negative
	// for a penumbral one.
	...
	// given; the list methods always produce English. A solar eclipse whose
	// peak is below the horizon is described at its deepest phase seen, at
	// sunrise or sunset, and called visible.
...
type MoonPhasesForYearOptions struct {
	...
	// ErrTimezoneResolutionFailed. Each boundary is local midnight at the
