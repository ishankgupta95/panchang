package i18n

var hi = PanchangTranslations{
	TithiNames: [14]string{
		"प्रतिपदा", "द्वितीया", "तृतीया", "चतुर्थी", "पंचमी",
		"षष्ठी", "सप्तमी", "अष्टमी", "नवमी", "दशमी",
		"एकादशी", "द्वादशी", "त्रयोदशी", "चतुर्दशी",
	},
	NakshatraNames: [27]string{
		"अश्विनी", "भरणी", "कृत्तिका", "रोहिणी", "मृगशिरा",
		"आर्द्रा", "पुनर्वसु", "पुष्य", "आश्लेषा", "मघा",
		"पूर्वा फाल्गुनी", "उत्तरा फाल्गुनी", "हस्त", "चित्रा", "स्वाति",
		"विशाखा", "अनुराधा", "ज्येष्ठा", "मूल", "पूर्वाषाढ़ा",
		"उत्तराषाढ़ा", "श्रवण", "धनिष्ठा", "शतभिषा", "पूर्वा भाद्रपद",
		"उत्तरा भाद्रपद", "रेवती",
	},
	YogaNames: [27]string{
		"विष्कम्भ", "प्रीति", "आयुष्मान", "सौभाग्य", "शोभन",
		"अतिगंड", "सुकर्मा", "धृति", "शूल", "गंड",
		"वृद्धि", "ध्रुव", "व्याघात", "हर्षण", "वज्र",
		"सिद्धि", "व्यतीपात", "वरीयान", "परिघ", "शिव",
		"सिद्ध", "साध्य", "शुभ", "शुक्ल", "ब्रह्म",
		"इंद्र", "वैधृति",
	},
	KaranaNames: KaranaNames{
		Movable: [7]string{"बव", "बालव", "कौलव", "तैतिल", "गर", "वणिज", "विष्टि"},
		Fixed:   [4]string{"किंस्तुघ्न", "शकुनि", "चतुष्पद", "नाग"},
	},
	VaraNames: [7]VaraName{
		{Name: "रविवार", Short: "रवि"},
		{Name: "सोमवार", Short: "सोम"},
		{Name: "मंगलवार", Short: "मंगल"},
		{Name: "बुधवार", Short: "बुध"},
		{Name: "गुरुवार", Short: "गुरु"},
		{Name: "शुक्रवार", Short: "शुक्र"},
		{Name: "शनिवार", Short: "शनि"},
	},
	PakshaNames: PakshaNames{Shukla: "शुक्ल", Krishna: "कृष्ण"},
	MasaNames: [12]string{
		"मेष", "वृषभ", "मिथुन", "कर्क",
		"सिंह", "कन्या", "तुला", "वृश्चिक",
		"धनु", "मकर", "कुंभ", "मीन",
	},
	ChandraMasaNames: [12]string{
		"चैत्र", "वैशाख", "ज्येष्ठ", "आषाढ़",
		"श्रावण", "भाद्रपद", "आश्विन", "कार्तिक",
		"मार्गशीर्ष", "पौष", "माघ", "फाल्गुन",
	},
	ChoghadiyaNames: [7]string{
		"उद्वेग", "चर", "लाभ", "अमृत", "काल", "शुभ", "रोग",
	},
	GowriNames: [8]string{
		"उद्योग", "अमृत", "रोग", "लाभ", "शुभ", "काल", "धन", "चल",
	},
	DoGhatiNames: [30]string{
		"रुद्र", "उरग", "मित्र", "पितर", "वसु",
		"अम्बु", "विश्वेदेव", "विधि", "ब्रह्म", "इन्द्र",
		"इन्द्राग्नि", "दैत्य", "वरुण", "अर्यमा", "भग",
		"ईश्वर", "अजैकपाद", "अहिर्बुध्न्य", "पूषा", "अश्विनी",
		"यम", "अग्नि", "ब्रह्म", "चन्द्र", "अदिति",
		"बृहस्पति", "विष्णु", "सूर्य", "त्वष्टा", "समीरण",
	},
	GrahaNames: [7]string{
		"सूर्य", "शुक्र", "बुध", "चंद्र", "शनि", "गुरु", "मंगल",
	},
	AnandadiYogaNames: [28]string{
		"आनन्द", "कालदण्ड", "धूम्र", "प्रजापति", "सौम्य",
		"ध्वांक्ष", "ध्वज", "श्रीवत्स", "वज्र", "मुद्गर",
		"छत्र", "मैत्र", "मानस", "पद्म", "लुम्बक",
		"उत्पात", "मृत्यु", "काण", "सिद्धि", "शुभ",
		"अमृत", "मूसल", "गद", "मातंग", "रक्ष",
		"चर्म", "स्थिर", "वर्धमान",
	},
	ChandraBalamNames: ChandraBalamNames{Shubha: "शुभ", Ashubha: "अशुभ"},
	TarabalaNames: [9]string{
		"जन्म",
		"सम्पत्",
		"विपत्",
		"क्षेम",
		"प्रत्यरि",
		"साधक",
		"वध",
		"मित्र",
		"अति-मित्र",
	},
	QualityNames:        QualityNames{Auspicious: "शुभ", Inauspicious: "अशुभ", Neutral: "सामान्य"},
	BhadraLocationNames: BhadraLocationNames{Earth: "पृथ्वी", Heaven: "स्वर्ग", Paatal: "पाताल"},
	PanchakaTypeNames: PanchakaTypeNames{
		Roga:    "रोग पंचक",
		Raja:    "राज पंचक",
		Agni:    "अग्नि पंचक",
		Chora:   "चोर पंचक",
		Mrityu:  "मृत्यु पंचक",
		Samanya: "सामान्य पंचक",
	},
	SpecialYogaNames: SpecialYogaNames{
		AmritSiddhi:     "अमृत सिद्धि योग",
		SarvarthaSiddhi: "सर्वार्थ सिद्धि योग",
		RaviPushya:      "रवि पुष्य योग",
		GuruPushya:      "गुरु पुष्य योग",
		Dwipushkar:      "द्विपुष्कर योग",
		Tripushkar:      "त्रिपुष्कर योग",
		Jwalamukhi:      "ज्वालामुखी योग",
		Aadal:           "आदल योग",
		Vidaal:          "विदल योग",
		Ravi:            "रवि योग",
	},
	Eclipse: EclipseTranslations{
		Template: "{subtype} {kind}: {percent}% ग्रस्त, {visibility}।",
		Kind:     EclipseKindNames{Solar: "सूर्य ग्रहण", Lunar: "चंद्र ग्रहण"},
		Subtype: EclipseSubtypeNames{
			Partial: "आंशिक", Total: "पूर्ण", Annular: "वलयाकार", Penumbral: "उपच्छाया",
		},
		Visibility: EclipseVisibilityNames{
			Visible:    "इस स्थान से दृश्य",
			NotVisible: "इस स्थान से अदृश्य",
		},
	},
	FestivalNames: map[string]string{
		"ugadi":                                "उगादी",
		"rama_navami":                          "राम नवमी",
		"hanuman_jayanti":                      "हनुमान जयंती",
		"akshaya_tritiya":                      "अक्षय तृतीया",
		"parashurama_jayanti":                  "परशुराम जयंती",
		"guru_purnima":                         "गुरु पूर्णिमा",
		"nag_panchami":                         "नाग पंचमी",
		"raksha_bandhan":                       "रक्षा बंधन",
		"krishna_janmashtami":                  "कृष्ण जन्माष्टमी",
		"ganesh_chaturthi":                     "गणेश चतुर्थी",
		"anant_chaturdashi":                    "अनंत चतुर्दशी",
		"navaratri":                            "शरद नवरात्रि",
		"durga_ashtami":                        "दुर्गा अष्टमी",
		"maha_navami":                          "महा नवमी",
		"dussehra":                             "दशहरा",
		"sharad_purnima":                       "शरद पूर्णिमा",
		"karva_chauth":                         "करवा चौथ",
		"dhanteras":                            "धनतेरस",
		"narak_chaturdashi":                    "नरक चतुर्दशी",
		"diwali":                               "दिवाली",
		"vasant_panchami":                      "वसंत पंचमी",
		"maha_shivaratri":                      "महा शिवरात्रि",
		"holika_dahan":                         "होलिका दहन",
		"holi":                                 "होली",
		"mahalaya_amavasya":                    "महालय अमावस्या",
		"kartika_purnima":                      "कार्तिक पूर्णिमा",
		"sankashti_chaturthi":                  "संकष्टी चतुर्थी",
		"onam":                                 "ओणम",
		"smarta_ekadashi":                      "स्मार्त एकादशी",
		"vaishnava_ekadashi":                   "वैष्णव एकादशी",
		"ekadashi_kamada":                      "कामदा एकादशी",
		"ekadashi_papamochani":                 "पापमोचिनी एकादशी",
		"ekadashi_mohini":                      "मोहिनी एकादशी",
		"ekadashi_varuthini":                   "वरुथिनी एकादशी",
		"ekadashi_nirjala":                     "निर्जला एकादशी",
		"ekadashi_apara":                       "अपरा एकादशी",
		"ekadashi_devshayani":                  "देवशयनी एकादशी",
		"ekadashi_yogini":                      "योगिनी एकादशी",
		"ekadashi_putrada":                     "पुत्रदा (पवित्रा) एकादशी",
		"ekadashi_kamika":                      "कामिका एकादशी",
		"ekadashi_parivartini":                 "परिवर्तिनी एकादशी",
		"ekadashi_aja":                         "अजा एकादशी",
		"ekadashi_pashankusha":                 "पाशांकुशा एकादशी",
		"ekadashi_indira":                      "इंदिरा एकादशी",
		"ekadashi_prabodhini":                  "प्रबोधिनी (देवउठनी) एकादशी",
		"ekadashi_rama":                        "रमा एकादशी",
		"ekadashi_mokshada":                    "मोक्षदा एकादशी",
		"ekadashi_utpanna":                     "उत्पन्ना एकादशी",
		"ekadashi_pausha_putrada":              "पौष पुत्रदा एकादशी",
		"ekadashi_saphala":                     "सफला एकादशी",
		"ekadashi_jaya":                        "जया एकादशी",
		"ekadashi_shattila":                    "षट्तिला एकादशी",
		"ekadashi_amalaki":                     "आमलकी एकादशी",
		"ekadashi_vijaya":                      "विजया एकादशी",
		"ekadashi_padmini":                     "पद्मिनी एकादशी",
		"ekadashi_parama":                      "परमा एकादशी",
		"ravi_pradosha":                        "रवि प्रदोष",
		"som_pradosha":                         "सोम प्रदोष",
		"bhauma_pradosha":                      "भौम प्रदोष",
		"saumya_pradosha":                      "सौम्य प्रदोष",
		"guru_pradosha":                        "गुरु प्रदोष",
		"bhrigu_pradosha":                      "भृगु प्रदोष",
		"shani_pradosha":                       "शनि प्रदोष",
		"baisakhi":                             "बैसाखी",
		"vishu":                                "विषु",
		"pohela_boishakh":                      "पोहेला बोइशाख",
		"puthandu":                             "पुथांडु",
		"bohag_bihu":                           "बोहाग बिहू",
		"makar_sankranti":                      "मकर संक्रांति",
		"pongal":                               "पोंगल",
		"uttarayan":                            "उत्तरायण",
		"magh_bihu":                            "माघ बिहू",
		"ayyappa_makara_jyothi":                "अय्यप्पा मकर ज्योति",
		"dakshinayana":                         "दक्षिणायन",
		"raja_sankranti":                       "राजा संक्रांति",
		"raja_pahili":                          "पहिली राजा",
		"raja_basi":                            "बासी राजा",
		"harela":                               "हरेला",
		"singh_sankranti":                      "सिंह संक्रांति",
		"sair":                                 "सैर",
		"kati_bihu":                            "काती बिहू",
		"lohri":                                "लोहड़ी",
		"gudi_padwa":                           "गुड़ी पड़वा",
		"gangaur":                              "गणगौर",
		"karaga":                               "करगा",
		"bonalu":                               "बोनालु",
		"hariyali_teej":                        "हरियाली तीज",
		"kajari_teej":                          "कजरी तीज",
		"hartalika_teej":                       "हरतालिका तीज",
		"govardhan_puja":                       "गोवर्धन पूजा",
		"bhai_dooj":                            "भाई दूज",
		"phagli":                               "फाग्ली",
		"jagannath_rath_yatra":                 "जगन्नाथ रथ यात्रा",
		"varamahalakshmi":                      "वरमहालक्ष्मी व्रत",
		"bathukamma_start":                     "बतुकम्मा: एंगिली पुला",
		"bathukamma_saddula":                   "बतुकम्मा: सद्दुला",
		"chhath_nahay_khay":                    "छठ: नहाय खाय",
		"chhath_kharna":                        "छठ: खरना",
		"chhath_sandhya_arghya":                "छठ: संध्या अर्घ्य",
		"chhath_usha_arghya":                   "छठ: उषा अर्घ्य",
		"vat_savitri_amavasya":                 "वट सावित्री (अमावस्या)",
		"vat_savitri_purnima":                  "वट सावित्री (पूर्णिमा)",
		"yajur_upakarma":                       "यजुर् उपाकर्म (आवणि अवित्तम्)",
		"rig_upakarma":                         "ऋग् उपाकर्म",
		"sama_upakarma":                        "साम उपाकर्म",
		"masik_shivaratri":                     "मासिक शिवरात्रि",
		"masik_karthigai":                      "मासिक कार्तिकै",
		"karthigai_deepam":                     "कार्तिकै दीपम्",
		"vinayaka_chaturthi":                   "विनायक चतुर्थी",
		"ravi_pushya":                          "रवि पुष्य",
		"guru_pushya":                          "गुरु पुष्य",
		"shravan_somvar":                       "श्रावण सोमवार",
		"mangala_gauri":                        "मंगला गौरी व्रत",
		"kartik_somvar":                        "कार्तिक सोमवार",
		"magha_shanivar":                       "माघ शनिवार",
		"surya_grahan":                         "सूर्य ग्रहण",
		"chandra_grahan":                       "चंद्र ग्रहण",
		"desc_ekadashi_viddha_vaishnava_next":  "दशमी-विद्धा: स्मार्त व्रत आज; वैष्णव व्रत अगले दिन (द्वादशी)।",
		"desc_ekadashi_viddha_vaishnava_today": "दशमी-विद्धा एकादशी: वैष्णव व्रत आज (द्वादशी)।",
		"desc_ekadashi_vriddha_dwadashi_next":  "वृद्धा द्वादशी: स्मार्त व्रत आज; वैष्णव व्रत अगले दिन (द्वादशी)।",
		"desc_ekadashi_vriddha_dwadashi_vaishnava": "वृद्धा द्वादशी: वैष्णव व्रत आज; स्मार्त व्रत कल था।",
		"desc_purnimanta_krishna_paksha":           "पूर्णिमांत: {masa} कृष्ण पक्ष",
		"desc_bhadra_observe_after":                "भद्रा समाप्ति के बाद ({time})",
	},
	Misc: MiscNames{
		Purnima:   "पूर्णिमा",
		Amavasya:  "अमावस्या",
		Adhika:    "अधिक",
		Ekadashi:  "एकादशी",
		Pradosha:  "प्रदोष व्रत",
		Sankranti: "संक्रांति",
	},
}
