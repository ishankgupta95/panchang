...
describe('computeTithiFromLongitudes', () => {
  ...
  it('Purnima at angle just under 180°', () => {
    const t = computeTithiFromLongitudes(179.9, 0, 'Purnima', 'Shukla');
  ...
  it('Amavasya at angle ~354°', () => {
    const t = computeTithiFromLongitudes(354, 0, 'Amavasya', 'Krishna');
    expect(t.index).toBe(29);
    expect(t.paksha).toBe('Krishna');
    expect(t.number).toBe(15); // Amavasya is 15th in Krishna
  });

  it('wraps when Moon < Sun (Moon at 10°, Sun at 350°)', () => {
    const t = computeTithiFromLongitudes(10, 350, 'Dwitiya', 'Shukla');
  ...
  it('completionPercentage is between 0 and 100', () => {
    for (let i = 0; i < 30; i++) {
      const angle = i * 12 + 6;
      const t = computeTithiFromLongitudes(angle, 0, 'test', 'test');
