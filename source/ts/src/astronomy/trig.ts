...
/* The reduction is written out in both functions rather than shared through module-level
 * scratch: the same operations in the same order, held in locals V8 keeps unboxed. */

/** Valid for |x| below ~10⁵, the range these series produce. */
export function sin(x: number): number {
  const q = Math.round(x * INVERSE_PI);
  const r = ((x - q * PI_1) - q * PI_2) - q * PI_3;
  const r2 = r * r;
