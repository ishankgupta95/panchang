...
/**
 * Night Choghadiya is **not** the day cycle restarted at a different name —
 * the night slots advance through their own succession:
 *
 *   Udveg → Shubh → Amrit → Char → Rog → Kaal → Labh → (repeat)
 *
 * distinct from the day order (Udveg → Char → Labh → Amrit → Kaal → Shubh →
 * Rog). An earlier version reused the day succession at night with only the
 * start name varied, which matched DrikPanchang on barely 2 of 8 night slots;
 * a later capture (2026-08-13) mis-transcribed Kaal/Labh order and Tuesday's
 * start. The tables below were re-verified against 14 consecutive drik nights
 * across two cities (Bengaluru Feb-2027 and Ujjain Aug-2026, zero exceptions):
 * starts Sun Shubh, Mon Char, Tue Kaal, Wed Udveg, Thu Amrit, Fri Rog,
 * Sat Labh — each weekday's start advances +2 (mod 7) along the succession.
 */
