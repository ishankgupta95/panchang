...
This exists because the v5 ephemeris port replaces the arithmetic under
8,000-odd pinned assertions, and when one of them fails there are three
...
| **1** | the reference almanac, ProKerala, AstroSage, PyJHora | Rule-level reference. Published to the minute, with their own rule interpretations | Only for *rule* questions, never for ephemeris accuracy |
...
### Where the almanac is *not* the right reference

The reference almanac is Tier 1 by construction, but for **moonrise and moonset**
it is not a reference at all, and the reason is worth stating so nobody chases it
as a bug:
...
| interpolated rise/set vs direct | `differential-riseset.test.ts` | Sun 1.7 / Moon 4.0 ms below 65°, 29.0 ms including the polar sites (worst at McMurdo, −77.9°) |
...
| solar eclipses vs NASA/Espenak | `tier0-own-eclipses.test.ts` | γ ≤0.00012 Earth radii in 452; local contacts ≤43.0 s (1901-2002), bias −0.85 s |
| rise/set vs USNO | `tier0-usno-riseset.test.ts` | Sun ≤29.7 s, Moon ≤37.5 s, bias ≤3 s, 0 event-presence mismatches (USNO's own resolution is ±30 s) |
| reference-almanac parity (Tier 1) | `element-endtime-audit.test.ts` | ≤58 s worst-case end-time drift |
