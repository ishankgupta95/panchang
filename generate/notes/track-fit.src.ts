/**
 * Worst angular separation, in arcsec, between the Chebyshev interpolant and
 * direct evaluation, behind `riseSet.ts`'s TRACK_BLOCK_DAYS and TRACK_NODES.
 * The floor is the millisecond quantization of `new Date()`.
 *
 *   npx esbuild generate/notes/track-fit.src.ts --bundle --platform=node --format=esm \
...
function sepArcsec(u: [number, number, number], v: [number, number, number]): number {
  // Not acos: it bottoms out at ~6e-3 arcsec, above the error measured here.
